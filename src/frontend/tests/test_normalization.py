...
class TestTokenize:
    ...
    def test_char(self):
        ...
        assert tokenized_text == '{"time": 0, "type": "word", "start": 0, "end": 1, "value": "b"}'

    def test_char_foreign(self):
        tokenized_text: str = [word.to_json() for word in _tokenize("д")][0]
        assert tokenized_text == '{"time": 0, "type": "word", "start": 0, "end": 2, "value": "д"}'

    def test_word_01(self):
        tokenized_text: str = [word.to_json() for word in _tokenize("appelsínusafi")][0]
        assert tokenized_text == '{"time": 0, "type": "word", "start": 0, "end": 14, "value": "appelsínusafi"}'

    def test_word_02(self):
        tokenized_text: str = [word.to_json() for word in _tokenize("köngull")][0]
        assert tokenized_text == '{"time": 0, "type": "word", "start": 0, "end": 8, "value": "köngull"}'

    def test_short_sentence_01(self):
        tokenized_text: Iterable[str] = [word.to_json() for word in _tokenize("Mennirnir notuðu áttavita.")]
        assert tokenized_text == [
    ...
    def test_short_sentence_02(self):
        tokenized_text: Iterable[str] = [word.to_json() for word in _tokenize("Hann hafði hlotið mörg verðlaun.")]
        assert tokenized_text == [
    ...
    def test_sentence_01(self):
        tokenized_text: Iterable[str] = [word.to_json() for word in _tokenize("Álfarnir lentu í ógöngum þegar þeir fóru í gegnum skóginn, enda óvanir háum trjám og mannætuköngulóm.")]
        assert tokenized_text == [
    ...
    def test_sentence_02(self):
        tokenized_text: Iterable[str] = [word.to_json() for word in _tokenize("Stjórnmálamönnunum þótti það ekki tiltökumál að samþykkja þingsályktunartillöguna, jafnvel þótt hún var talin illa ígrunduð af minnihlutanum.")]
        assert tokenized_text == [
    ...
    def test_sentence_foreign_char(self):
        tokenized_text: Iterable[str] = [word.to_json() for word in _tokenize("Það þótti ekki nógu kдrlmannlegt.")]
        assert tokenized_text == [
    ...
    def test_num_char(self):
        ...
        assert tokenized_text == '{"time": 0, "type": "word", "start": 0, "end": 2, "value": "15"}'

    def test_sentence_num_char(self):
        tokenized_text: Iterable[str] = [word.to_json() for word in _tokenize("Klukkan var langt gengin í 7.")]
        assert tokenized_text == [
